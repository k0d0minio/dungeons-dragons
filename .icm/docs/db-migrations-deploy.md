# Database migrations on deploy — runbook (DND-013, DND-024)
...
> Deleted with the 2026-08-15 board reset (D9), restored and corrected by
> DND-024 — both workflows had cited it throughout, including the failure
> handler that points here mid-incident.
...
Nothing below is in git. The two workflows treat absence differently, and the
difference is deliberate (DND-024):

- **Production** (`DATABASE_URL`): the migrate job **fails** if it is missing.
  It is set today; if it ever rots, the red job is the alarm.
- **Preview** (`NEON_API_KEY` / `NEON_PROJECT_ID`): the job skips with a notice.
  Jamie chose on 2026-08-15 **not** to set these, accepting that migrations are
  first applied against production on merge — the same acceptance that let
  `drizzle/0001` fail there (run 31910353352, rolled back cleanly). Setting the
  two values is the entire fix if that stops being acceptable; no workflow edit
  needed.
...
_Chosen because_ Jamie is both the author and the only reviewer here — a gate
that only he can clear, on work only he wrote, is ceremony rather than safety.

_To add the gate later_: Settings → Environments → `production` → Required
...
_The alternative, if that stops being acceptable_: set
...
2. **Fix the schema in `src/lib/db/schema.ts`,** regenerate with
   `npm run db:generate`, and delete the bad migration file _and its snapshot
   entry_ — do not leave a broken migration in `drizzle/` for someone to trip
   over later.
