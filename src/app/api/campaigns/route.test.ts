...
describe('POST /api/campaigns', () => {
  ...
  // Creating no longer carries a table forward: that is
  // `PUT /api/campaigns/[id]/carry-from` (`triage/carry-forward-rerun`), so a
  // body still carrying the old key is ignored rather than acted on.
  it('ignores a carryFrom left over from the old one-call shape', async () => {
    signedIn()

    const response = await POST(
      jsonRequest({ name: 'The real one', carryFrom: '9c3d5e2b-4f6a-4b7c-9d0e-1f2a3b4c5d6e' }),
    )

    expect(response.status).toBe(201)
    expect(mockCreateCampaign).toHaveBeenCalledWith(DM, 'The real one')
  })
})
