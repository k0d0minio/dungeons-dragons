...
//
// Creating is creating, and nothing else. Carrying a table forward used to be
// a `carryFrom` on this body; it is `PUT /api/campaigns/[id]/carry-from` now
// (`triage/carry-forward-rerun`), which the create form calls straight after
// this one. Two calls rather than one, on purpose: the carry is then an act
// that can be run again against the campaign this made, instead of a half of
// this one that could only be retried by making a second campaign.
